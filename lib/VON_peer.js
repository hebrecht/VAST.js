...
const VAST_Voronoi = require('./voronoi/vast_voro.js');
// voronoi computation
...
function VONPeer(l_aoi_buffer, l_aoi_use_strict) {
    ...
    this.areaMessage = function(areaPacket, callback) {
        ...
        //console.log('areaMessage targets: '+VON_pack.targets)
        _sendPack(VON_pack, true);
        ...
        /*
        var closest = _voro.closest_to(areaPacket.targetAoI.center);
        if (closest !== _self.id){
            var VON_pack = new VAST.pack(VON_Message.MATCHER_AREA, areaPacket, VAST.priority.HIGHEST);
            VON_pack.targets.push(closest);
            _sendPack(VON_pack, true);
        } else {
            this.forwardOverlapping(areaPacket);
        }
        */
    }
    ...
    this.forwardOverlapping = function(areaPacket, callback){
        // get an array of overlapping neighbours (list of pointNodes)
        var overlapping = _getOverlappingNeighbours(areaPacket.targetAoI);

        // array of node IDs that have already received the message
        ...
        if (!recipients.includes(_self.id))
        ...
        // sort recipients and update
        /* sort really neccessary?
        ...
        */
        ...
        switch (pack.type) {
            ...
            case VON_Message.MATCHER_POINT: {
                //console.log('Received message MATCHER_POINT')
            ...
            case VON_Message.MATCHER_AREA: {
              ...
              //console.log(pack.msg);
                ...
                // The center was found, now forwarding to overlapping neighbours 
                    ...
                    //console.log('I am the closest matcher')
                    this.forwardOverlapping(pack.msg, function(updatedPacket) {
                        ...
                        (typeof _my_matcher.handlePacket === 'function')) {
                            //console.log('My matcher should handle this packet from: '+from_id)
